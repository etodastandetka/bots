...
try:
    ...
    BOT_TOKEN = BOT_TOKENS["mostbet"]
    ADMIN_BOT_TOKEN = BOT_TOKENS["admin"]
except ImportError:
    # Fallback значения если config.py не найден
    API_CONFIG = {
        "api_key": "d6ad6a2a6a578d10a47d475eb8475ed60337d96e8b3d157d285ce3328320de76",  # API ключ Mostbet
        "deposits_blocked": False  # Разрешаем пополнения для Mostbet
    }
...
translations = {
    'ru': {
        ...
        'enter_id': "🆔 Отправьте ID вашего счета 1xBet",
        ...
        'enter_1xbet_id': "Введите ID вашего счета 1xBet",
        ...
        'enter_account_id': "Введите ID вашего счета 1xBet:",
        ...
        'enter_account_id_prompt': "📱 Введите ID вашего счета 1xBet:",
        'example_id_title': "",
        'find_id_instruction': "",
        ...
        'enter_1xbet_id_withdraw': "Введите ID 1xBet:",
...
class Database:
    def __init__(self, db_path: str = '1xbet_bot.db'):
    ...
    def set_user_xbet_id(self, user_id: int, xbet_id: str, bot_source: str = '1xbet'):
    ...
    def add_user_xbet_id(self, user_id: int, xbet_id: str, bot_source: str = '1xbet'):
        """Добавить новый ID для пользователя (для обратной совместимости)"""
        self.set_user_xbet_id(user_id, xbet_id, bot_source)
    
    def get_user_xbet_id_single(self, user_id: int, bot_source: str = '1xbet') -> Optional[str]:
    ...
    def get_user_xbet_ids(self, user_id: int, bot_source: str = '1xbet') -> List[str]:
    ...
    def remove_user_xbet_id(self, user_id: int, xbet_id: str, bot_source: str = '1xbet'):
    ...
    def set_user_phone(self, user_id: int, phone: str, bot_source: str = '1xbet'):
    ...
    def get_user_phone(self, user_id: int, bot_source: str = '1xbet') -> Optional[str]:
...
class OneWinAPI:
    def __init__(self):
        self.base_url = "https://api.1xbet.win"
        self.api_key = None
        self.session = None
    
    def set_api_key(self, api_key: str):
        """Установка API ключа для 1xBet"""
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    logger.error(f"1xBet API deposit error: {response.status} - {error_text}")
                    return None
                    
        except Exception as e:
            logger.error(f"1xBet API deposit exception: {e}")
            return None
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"1xBet API withdrawal error: {response.status} - {error_text}")
                    return None
                    
        except Exception as e:
            logger.error(f"1xBet API withdrawal exception: {e}")
            return None
